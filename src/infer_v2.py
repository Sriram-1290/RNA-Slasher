...
def predict_single(X, bio_feats_dim, model_type='enhanced'):
    """
    Make a single prediction using specified model type
    ...
    if model_type in ['original', 'both']:
        ...
        if original_preds:
            results['original'] = np.mean(original_preds)
        else:
    ...
    if model_type in ['enhanced', 'both']:
        ...
        if os.path.exists(enhanced_model_path):
            try:
                ...
                with torch.no_grad():
                    x_tensor = torch.tensor(X, dtype=torch.float32).to(device)
                    results['enhanced'] = model(x_tensor).cpu().item()
            except Exception as e:
...
def predict_from_file(csv_file, model_type='enhanced', output_file=None):
    ...
    for i in range(len(pred_dataset)):
        ...
        pred = predict_single(x_input, bio_feats_dim, model_type)
    ...
    if model_type == 'both':
        df['original_prediction'] = [p.get('original') if isinstance(p, dict) else None for p in predictions]
        df['enhanced_prediction'] = [p.get('enhanced') if isinstance(p, dict) else None for p in predictions]
    else:
        df[f'{model_type}_prediction'] = predictions
...
def main():
    parser = argparse.ArgumentParser(description='RNA-Slasher Enhanced Inference Tool')
    ...
    if args.mode == 'interactive':
        ...
        print("=== RNA-Slasher Enhanced Inference ===")
